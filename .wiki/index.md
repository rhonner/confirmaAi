...
| [regression-test-assert-the-predicate](pages/concepts/regression-test-assert-the-predicate.md) | Grep da chamada (`findMany`) ≠ grep do predicado load-bearing (`!promotedIds.has`): check tautológico passa com o filtro invertido/removido. Asserte o predicado ou observe a saída real. **Corolário (2026-07-24):** `findFirst` sem `orderBy` escolhe linha arbitrária → flake (`P2002` no check seguinte) | 2026-07-24 |
...
| ------ | -------- | --------- |
| `raw/sessions/` | 23 | Sumários de sessões de trabalho. |
