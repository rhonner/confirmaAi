...
| [claude-chrome-per-profile-extension](pages/concepts/claude-chrome-per-profile-extension.md) | Extensão Claude-in-Chrome é por-perfil; deviceIds/nomes embaralham entre sessões → confirmar pela conta logada (WeCalc, nunca work), não pelo nome. **2026-07-25:** a conta "certa" pode **não existir em perfil nenhum** (só pessoal + work) → enumerar por `u/0|1|2` (redirect p/ `u/0` = acabou); sessão do app ≠ conta Google; se a conta da integração de prod é inalcançável, testar por injeção no cliente | 2026-07-25 |
...
| [external-event-firewall](pages/concepts/external-event-firewall.md) | Registros de fonte externa (Google Calendar) em **tabela separada só-leitura**, não coluna `source` em `Appointment` — senão o scheduler manda WhatsApp/marca NO_SHOW falso. Firewall físico > filtro que se esquece. **Fase B** (promoção + de-dup) + **Fase C** (firewall nos DOIS sentidos). **2026-07-24:** vale também p/ `TimeBlock` (tabela própria, não `Appointment` sem paciente) e **só-leitura ≠ inerte** — o evento é clicável (promove ou abre no Google), só não é mutável. **Tabela separada quando a linha não é de domínio; FLAG (`retroactive`) quando é**. **2026-07-25:** a política de promoção lia **copy** ("Ocupado") → passou a ler `isPrivate`; validada em produção | 2026-07-25 |
...
| [regression-test-assert-the-predicate](pages/concepts/regression-test-assert-the-predicate.md) | Grep da chamada (`findMany`) ≠ grep do predicado load-bearing (`!promotedIds.has`): check tautológico passa com o filtro invertido/removido. Asserte o predicado ou observe a saída real. **Corolários (2026-07-24):** `findFirst` sem `orderBy` → flake (`P2002` no check seguinte); e check **negativo** por grep tem de rodar sobre o fonte **sem comentários** (a doc que explica a remoção cita a string removida) | 2026-07-24 |
...
| [chrome-mcp-drive-and-assert-via-js](pages/concepts/chrome-mcp-drive-and-assert-via-js.md) | Técnicas de teste no Chrome MCP: setar select/input nativo via setter do prototype + `dispatch('change')` p/ RHF captar; interceptar `window.fetch` p/ asseverar payload e injetar latência p/ ver loading. **§5:** `resize_window` é no-op → não emula mobile. **§6 (2026-07-24):** injetar fixture no cliente + stubar `window.open` p/ testar caminho externo sem sujar a conta real do dono — **refinado 2026-07-25** (envelope real `data.events`; acumular `__opened` p/ provar "não abriu"; injetar em faixa não-cacheada; vale contra o bundle de PROD) | 2026-07-25 |
...
| [react-query-structural-sharing-defeats-prop-diff](pages/concepts/react-query-structural-sharing-defeats-prop-diff.md) | Refetch *deeply equal* devolve a **MESMA referência** → efeito "limpa estado otimista quando as props mudam" nunca dispara justo no cancelamento/erro (card preso na posição arrastada). Estado otimista precisa de sinal EXPLÍCITO de fim de tentativa (`.finally` numa promise que nunca rejeita) | 2026-07-24 |
| [move-across-days-via-local-components](pages/concepts/move-across-days-via-local-components.md) | Mover data entre dias = remontar por componentes locais (dia do destino + hora da origem), **nunca somar 24h**: no DST o dia local tem 23h/25h e o horário desliza. Isolar num helper puro e testar — não reproduz fora da transição | 2026-07-24 |
| [drag-vs-click-decide-by-value-change](pages/concepts/drag-vs-click-decide-by-value-change.md) | Tap × arraste decide-se pela **mudança real do valor com snap**, não por limiar de pixels (limiar < passo de snap → PUT no-op + escrita no Google por tremor de dedo). Junto: `touch-action:pan-y`, `pointercancel` aborta, flag p/ clique-fantasma, teclado via `detail===0`. **O custo do clique-fantasma depende do efeito do clique** — a célula do Mês passou a CRIAR (o drill migrou p/ o número do dia) | 2026-07-24 |
| [persist-intent-not-elapsed-time](pages/concepts/persist-intent-not-elapsed-time.md) | Intenção ("lancei no passado de propósito") **grava-se na escrita**; derivar de `dateTime < now` na leitura colapsa com "marquei p/ o futuro e o horário passou" — e o 2º é a falta que o produto mede. Flag só vale se os jobs filtrarem por ele | 2026-07-24 |
| [redacted-label-is-copy-not-contract](pages/concepts/redacted-label-is-copy-not-contract.md) | Rótulo redigido ("Ocupado" p/ evento particular) é **copy**, não contrato: política que compara o título quebra quando um redator renomeia — e aqui isso criaria paciente "Ocupado" **queimando vaga vitalícia**. Smell = **derivar e descartar**: o mapper já sabia (`visibility`) e jogou fora → exporte o predicado (`isPrivate`). Travar com asserção negativa no fonte | 2026-07-25 |
| [toast-timers-pause-on-hover](pages/concepts/toast-timers-pause-on-hover.md) | Sonner **pausa o auto-dismiss e expande a pilha no hover** → arraste que termina na faixa do toast congela os antigos e esconde o novo: parece "toast não disparou". Soltar longe / `hover` distante + espera; melhor ainda asseverar `[data-sonner-toast]` no DOM. Toast velho na tela não prova nada | 2026-07-25 |
| [audit-trail-proves-side-effect-absence](pages/concepts/audit-trail-proves-side-effect-absence.md) | Auditoria append-only responde "o sistema mandou WhatsApp?" quando o registro já foi excluído: **ausência** de `message.sent` **e** de `message.send_failed` = job nunca pegou. Exige logar o caminho de falha; e logar mutação de terceiro só no `ok` (`gcal.pushed`) transforma presença em prova | 2026-07-25 |
...
| [google-calendar-integration-state](pages/synthesis/google-calendar-integration-state.md) | Feature de core que destrava o PREMIUM. Faseamento A→B→C; firewall `ExternalEvent` (agora nos 2 sentidos); OAuth separado. **2026-07-25: A, B e C RODANDO EM PRODUÇÃO** (status `CONNECTED` + `mirrorActive`, promoção abre nas grades, espelho criado/apagado provado por `gcal.pushed`) — o tier segue **dark** (`hidden:true`). ⛳ GA pendente de **verificação OAuth do escopo de ESCRITA** + `hidden:false`; sync contínuo Google→app (B2) não iniciado | 2026-07-25 |
...
| ------ | -------- | --------- |
| `raw/sessions/` | 26 | Sumários de sessões de trabalho. |
