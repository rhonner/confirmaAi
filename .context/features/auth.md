...
### Login não-verificado mais amigável + polish de UI das telas de auth (2026-06-27 — feedback da sócia)

- **Mensagem amigável robusta (`(auth)/login/page.tsx`)**: a detecção do erro do `signIn` deixou de ser um `else` genérico que assumia "senha errada" para QUALQUER erro. Agora: `code = result.error.toUpperCase()`; `EMAIL_NOT_VERIFIED` → painel amigável; `CREDENTIALSSIGNIN` → "Email ou senha incorretos"; qualquer outro (Configuration/CSRF/infra) → "Não foi possível entrar agora. Tente novamente em instantes." (não afirma senha errada). O painel "Confirme seu e-mail" agora **rola para a vista + recebe foco** (`ref` + `useEffect` com `scrollIntoView`/`focus`, `tabIndex={-1}`, `scroll-mt-20`) — antes nascia acima da dobra e a sócia não o via. `handleResend` checa `res.ok` (4xx/5xx não viram "reenviado") e o copy ficou condicional (anti-enumeration). Validado no Chrome MCP: conta não-verificada → painel aparece focado, `callback/credentials 401`, `resend-verification 200`.
