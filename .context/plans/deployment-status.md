...
| 2 | Conta **Asaas produção** | **usuário** | 🟡 conta criada. **Webhook ✅ configurado 2026-06-10** (nome "ConfirmaAi - producao", URL `/api/billing/webhook`, v3, token = `ASAAS_WEBHOOK_SECRET`, sequencial + fila ativa, todos eventos de Cobranças). **Falta só a `ASAAS_API_KEY`**: botão "Gerar chave" está desabilitado até **regularizar o cadastro** — Minha Conta → aba Informações → preencher todos os dados incluindo faturamento (KYC; dados pessoais = usuário). Ativar NF-e **quando houver CNPJ** (decisão: começar como PF). Nota: bloqueio da extensão p/ sites financeiros foi prompt de permissão — aprovado, automação funcionou. |
