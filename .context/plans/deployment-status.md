...
| 8 | Smoke test E2E | dev + usuário | ⏳ próximo: login → badge/billing → QR WhatsApp (usuário escaneia) → paciente+agendamento → cron 30min envia → responder "1" → CONFIRMED. Opcional: assinar Pro com Pix R$ 1 real (validar webhook ativando) e cancelar/estornar. |
