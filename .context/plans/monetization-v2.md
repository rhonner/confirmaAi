...
- [ ] **Checkout retry cria assinaturas duplicadas no gateway** (achado 2026-06-12): cada `POST /api/billing/checkout` cria nova subscription no Asaas; falha no meio (ex: sem chave Pix → `pixQrCode 400 invalid_action`) deixa assinatura órfã pendente que continua gerando cobranças mensais. Fix: antes de criar, verificar `Subscription.providerSubscriptionId` pendente e reaproveitar/cancelar no provider. Mitigação manual: cancelar órfãs no painel Asaas.
