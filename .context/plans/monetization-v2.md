...
- [x] **(fatia 2.3, 2026-06-20) Dunning + perto-do-limite**: camada de comunicação no cron via `runBillingNotifications()` (`src/lib/services/billing-notifications.ts`), espelhando o padrão Sprint 8 (fns puras + dedup por audit, **sem migration**). **Dunning**: emails nos dias 1/3/7 do PAST_DUE (`dunningStageDue` — só o maior marco vencido ainda não enviado; âncora = 1º `billing.payment.failed` desde o último `billing.payment.received`, fallback `updatedAt`), DAY_7 com aviso de suspensão iminente. Roda **ANTES** do `billing-maintenance` (senão a suspensão >7d engoliria o email do dia 7). **Perto-do-limite**: ao cruzar 80%/100% das mensagens do período (`usageThresholdDue`), 1 email/marco/período (dedup por `periodStart` no audit, **Free incluído**). Audits `billing.dunning.sent` / `billing.usage.threshold_notified` (+ labels). Régua: 229 vitest (+15), 109 sprints (10.15/10.16), integração no DB validada (DAY_7 + 80% enviados e dedupados). Ver [`../features/billing.md`](../features/billing.md) e [`../features/scheduler.md`](../features/scheduler.md).
- [x] **(fatia 2.2, 2026-06-14)** Emails transacionais event-driven (Resend), via `src/lib/emails/transactional.ts` (builders puros + senders, reusam o layout): **boas-vindas** (verify-email pós-ativação), **pagamento confirmado** (webhook PAYMENT_RECEIVED, try/catch ISOLADO pra não virar `apply_failed`), **cancelamento** (`/api/billing/cancel`). Todos best-effort (falha de email não trava o fluxo) + `captureError`. Régua: 199 vitest (+5 builders, incl. anti-double-escape), 100 sprints (+10.7). _("próximo do limite" movido pra 2.3 com o dunning — ambos stateful/dedup.)_
...
- [x] **(2026-06-20)** Reset de conta Free (1×, com check de zero agendamentos) — `POST /api/account/reset` + card destrutivo em `/configuracoes` (só FREE). Guardas: FREE + **zero agendamentos (qualquer status)** + 1× via dedup de audit (`account.reset`, sem migration). Limpa Patient (cascade Appointment/MessageLog) + PatientQuotaSlot + zera `patientSlotCount`. `canResetFreeAccount` exposto na subscription gateia a UI. Ver [`../features/account-reset.md`](../features/account-reset.md).
- [ ] Onboarding banner com upgrade CTA.
- [x] **(2026-06-20) Checkout com `User.cpf` null** (bug achado no teste de pagamento real 2026-06-12): Asaas criava o customer sem CPF e rejeitava a assinatura (`400: "preencher o CPF ou CNPJ do cliente"`); afetava contas grandfathered (pré-Sprint 4). **Fix entregue**: helper puro `resolveCheckoutCpf` + `POST /api/billing/checkout` aceita `cpf` opcional → quando `user.cpf` null e nada informado, retorna `400 CPF_REQUIRED` **antes** de tocar o provider (não cria mais customer órfão); informado → valida (`validateCpf`), persiste `User.cpf/cpfHash`, e `provider.updateCustomer` (Asaas `POST /customers/{id}`) sincroniza o CPF no customer existente. UI mostra campo CPF ao receber `CPF_REQUIRED`. **Hardening da review adversarial**: (a) o branch de persist agora aplica os MESMOS controles anti-fraude do register (hard-block `>=4` → `CPF_LIMIT` 409 + `detectOwnerCpfReuse` com auto-suspend `>3`) — antes era uma porta pra burlar a dedup de CPF do dono; (b) `updateCustomer` virou idempotente (roda em todo checkout com customer existente, não one-shot) pra recuperar customer órfão sem CPF mesmo após falha de rede. 207 vitest + 102 sprints (10.8/10.9), validado no Chrome (FREE+cpf null → CPF_REQUIRED → campo → QR; CPF persistido + audit `billing.checkout.cpf_added`). Ver [`../features/billing.md`](../features/billing.md).
- [x] **(2026-06-20) Checkout retry cria assinaturas duplicadas no gateway** (achado 2026-06-12): cada `POST /api/billing/checkout` criava nova subscription no Asaas; retry/abandono deixava assinatura órfã pendente gerando cobrança mensal. **Fix entregue**: nova primitiva `provider.cancelSubscription(providerSubscriptionId)` (Asaas `DELETE /subscriptions/{id}`, Mock no-op); o checkout, antes de criar a nova assinatura, **cancela a pendente** referenciada quando não há assinatura paga ativa em vigor (`plan === FREE || status !== ACTIVE`) + audit `billing.subscription.replaced` (best-effort, não bloqueia o checkout). **Bônus (mesma primitiva)**: a rota `/api/billing/cancel` passou a cancelar no provider (fecha o TBD "cancelar no Asaas" — antes o gateway seguia cobrando após cancelamento). Validado contra o **sandbox real** (sub criada → `cancelSubscription` → `deleted:true/INACTIVE`). 208 vitest + 103 sprints (10.10). Órfãs já existentes em prod = limpeza manual (ver deployment-status). Ver [`../features/billing.md`](../features/billing.md).
- [ ] **Onboarding Asaas — chave Pix**: documentar no go-live checklist que a conta Asaas precisa de chave Pix cadastrada (Pix → Minhas chaves → aleatória) ANTES do primeiro checkout Pix; sem ela o QR falha com `invalid_action`. (Feito manualmente em 2026-06-12.)
- [x] **(2026-06-20, Opção A) Expiração curta do QR Pix + regenerar** — `src/lib/billing/pix-ttl.ts` (`PIX_QR_TTL_SECONDS` default 300s/5min, calibrável) define o `expiresAt` curto do checkout; o front mostra countdown e, ao expirar, "Gerar novo QR" → `POST /api/billing/checkout/refresh` → `provider.refreshPixCharge` re-busca a cobrança Pix da MESMA assinatura (sem criar nova; audit `billing.checkout.qr_refreshed`). Asaas mantém o QR válido ~12 meses, então o TTL é política do produto (Opção A = visual; Opção B/invalidar descartada). Régua: vitest + 109→ sprints (10.17-10.19), validado no Chrome. Ver [`../features/billing.md`](../features/billing.md). _(histórico do backlog abaixo)_
  - ~~hoje a cobrança Pix nasce com janela longa~~ — `asaas.ts` cria a cobrança com `nextDueDate = hoje + 3 dias` e o QR (`GET /payments/{id}/pixQrCode`, campo `expirationDate`) herda essa validade. Efeito: o QR/cobrança fica **aberto muito tempo no gateway** — cliente pode pagar um código velho e cobranças pendentes acumulam (agrava o item "checkout retry duplica assinatura" acima). **Desejado**: no checkout, dar ao QR um **TTL curto (~3 min — valor a calibrar)** com countdown; ao expirar, **cancelar/expirar a cobrança Pix atual no Asaas** (`DELETE /payments/{id}` ou definir expiração) e **gerar uma nova cobrança + QR reusando a MESMA `Subscription`/`providerSubscriptionId`** (NÃO criar nova assinatura — senão cai no bug do retry duplicado). Pontos de implementação: (a) frontend `src/app/(dashboard)/billing/checkout/page.tsx` — countdown a partir do `expiresAt` já retornado pelo checkout + botão (ou auto) "Gerar novo QR"; (b) backend — endpoint "refresh QR" (ou flag no `POST /api/billing/checkout`) que regenera só a cobrança avulsa, sem tocar na assinatura; (c) verificar na API Asaas se dá pra encurtar `dueDate`/expiração numa assinatura recorrente vs precisar de cobrança avulsa separada.
- [x] **🔴 BUG CRÍTICO — externalReference em `payment` (corrigido 2026-06-12)**: o webhook `PAYMENT_RECEIVED` chegava e processava OK, mas o plano nunca subia de FREE — o handler procurava `externalReference` em `payload.subscription` e no topo, enquanto o Asaas o envia em **`payload.payment.externalReference`** (`<userId>:PRO`). Efeito em produção: TODO cliente pagaria e continuaria no Free. Achado no teste de pagamento real (Pix R$ 3). Fix: helper `planTierFromPayload` (provider.ts) lê as 3 fontes; 5 testes de regressão com shape real do Asaas. Conta teste reconciliada manualmente para PRO.
